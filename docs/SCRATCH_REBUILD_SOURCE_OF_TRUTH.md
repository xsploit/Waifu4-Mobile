...
| Main assistant Tavily tools | VERIFY | Account-tab Tavily key routing feeds `/ai/chat`; main chat attaches AI SDK `tavily_search`, `tavily_extract`, and `tavily_crawl` tools with bounded steps and OpenRouter `require_parameters=true`. GRILLO and main-chat Tavily schemas share `src/lib/grillo/tavily-tool-definitions.ts`. Needs real-key runtime verification. |
| TTS core | DONE | Fish realtime/current, Fish timestamp SSE, Inworld HTTP/WebSocket, benchmark mode, unified timing metadata. Copied frontend remote TTS requests map directly to the rebuilt `/tts/stream` schema and expose Fish WebSocket/timestamp-SSE plus Inworld HTTP/WebSocket transports and provider micro-parameters without backend behavior changes. The copied TTS tab now restores the browser-audible benchmark surface from the earlier simple demo, and active remote PCM playback now uses benchmark-style scheduling plus direct `source -> destination` output. Provider word timing feeds subtitles when present; estimated subtitles remain the fallback. |
| Mouth core | DONE | `wlipsync` live path works from the playback audio source and remains the mouth owner. Provider timing is normalized for captions now; provider phoneme/viseme-driven mouth flaps remain a deliberate future option, not the active path. |
...
| Account/local transfer backup surface | DONE | Copied Account tab provider-key vault and local transfer backup export/import are wired into the active UI. Import/export failures now report in the Account status instead of escaping the async click/file handlers; the known local backup shape parses with provider keys, saved VRMs, personas, and active tab. |
| Legacy local backup normalization | DONE | Local transfer import accepts current `web-waifu-4-local` plus legacy `yourwifey-local`/`yourwifey-byok` envelopes; missing `includes` blocks are derived from normalized chat history, provider keys, relationship memory, and saved VRMs instead of silently showing false flags. |
| Persona scoped import context | DONE | Local transfer import now restores the active persona's scoped chat history and relationship memory immediately, falling back to legacy global fields only when scoped data is absent. This preserves the persona/context separation rule during backup restore. |
| Twitch backend transcription/frame foundation | COPIED | Old IRC parser/source and stream transcriber copied as optional backend plumbing; `/twitch/transcribe-sample` and `/twitch/capture-frame` are wired to current backend key headers/env. Frontend direct IRC remains the primary public chat intake. |
| Twitch stream vision contract | DONE | Copied frontend frame capture can attach a fresh stream JPEG as optional user-message image context, and the rebuilt backend `/ai/chat` contract maps those images through the AI SDK provider boundary for vision-capable models. |
...
| Direct VRM stage/settings surface | DONE | Old direct `VrmStage`, settings tabs, chat overlay, and menu shell are now the active frontend surface. Mouth/TTS ownership is patched through backend Fish live bridge audio events plus the copied `wlipsync` playback tap. Model scale changes are applied without tearing down and reloading the active VRM. |
| Animation idle sequencing | DONE | Main autoplay now defaults to shuffle and uses an unweighted non-repeating random bag. The old per-animation chance control is removed from the active Animation tab. |
| VRM expression writer priority | DONE | Animation-loaded expression tracks now strip protected mouth/blink/look keys so `wlipsync`, autoblink, and procedural gaze keep ownership; emotion/facial expressions still resolve only non-reserved expressions. |
| VRM expression telemetry | DONE | Emotion Log now includes a throttled live VRM snapshot showing final `aa/ih/ou/ee/oh` mouth weights plus active non-mouth expression weights, so mouth/expression priority can be inspected from the copied settings surface. |
...
| Twitch command/overlay/scheduler foundation | ADAPT | Old server command parser/router, overlay socket, chat scheduler/message filters, mock Twitch source, and tests are copied into `server/*` and patched for the rebuild imports. Overlay socket is attached to the backend `/ws`; the active frontend listens by default so backend runtime command broadcasts reach the copied command handlers, while `VITE_STREAM_BOT_WS_ENABLED=false`/`VITE_OVERLAY_WS_ENABLED=false` can still disable it for static runs. Frontend direct IRC intake is active and covered for anonymous connect/PING/tagged `PRIVMSG`; frontend command parsing uses a tested shared parser with copied command grammar plus frontend-only controls. Optional backend runtime wires mock/IRC source -> command router -> scheduler -> overlay broadcasts and exposes runtime status/start/stop/mock-message endpoints. Backend IRC now forwards JOIN/PART membership events through the overlay socket for the same Twitch-tab status hook. Backend command routing now matches the direct frontend persona/character controls (`personas`, `persona <name>`, `character <name>`) through the overlay command surface. Backend IRC remains optional/fallback plumbing, not the primary frontend chat intake. |
...
| `src/lib/product` backup/key vault/account | 9 | Active copy | Backup/key vault/account files copied and wired into the active Account tab; local transfer backup import/export has focused tests and user-visible error status. |
...
| `src/lib/twitch` direct IRC/transcription helpers | 5 | Active copy | Direct browser IRC, stream transcription helpers, and shared frontend command parser are active. Direct IRC is the primary public chat intake and is covered for anonymous `justinfan` connect, PING/PONG, tagged `PRIVMSG` parsing, membership JOIN/PART events surfaced as Twitch-tab status, and command grammar; transcription/frame endpoints are wired through the local backend. |
| `src/lib/vrm` loader/animation/sequencer/custom library | 9 | Active copy/adapt | Loader/custom library/postprocessing/animation/sequencer copied; active `VrmStage` copied/adapted and mounted. Protected mouth/blink/look expression ownership is enforced at animation load; broader custom expression authoring/debug mixer work remains later. |
| `server/src/twitch` IRC/transcriber | 5 | Partial active copy | IRC parser/source and stream transcriber copied; transcription/frame routes wired. Backend IRC runtime is optional/fallback plumbing, not the primary frontend chat intake. Captured frames can now flow through `/ai/chat` as optional image context for vision-capable models. |
...
| `server/src/overlay` socket/events | 2 | Active optional backend copy | Overlay socket copied to `server/overlay`, attached to backend `/ws`, receives command/scheduler runtime broadcasts, and is consumed by the active frontend by default. |
...
| AI | LLM provider/model/settings, capability metadata, health, generation knobs, tools/reasoning controls. | ACTIVE / VERIFY | Uses rebuilt backend contracts and model metadata. Main assistant Tavily web tools are wired through the existing Account-tab browser key path and AI SDK tool loop; current remaining check is real-key runtime verification on OpenRouter and Vercel Gateway. GRILLO/Ladybug memory-worker tools remain separate. |
...
| Voice Lab | Persona voice binding and provider voice creation/saved voice management. | ACTIVE / VERIFY | UI is wired to active voice bindings plus `/tts/voices` and `/tts/voices/create` backend routes copied/adapted from the old project. Live provider cloning/listing still needs real-key Fish/Inworld verification per provider. |
...
- **D11 — Finish the LLM capability layer before TTS.** The manual `replyFormat` dropdown is a "decide by vibes" placeholder. Correct order: (1) `GET /ai/models`, (2) fetch OpenRouter model metadata (`supported_parameters` + `top_provider.supported_parameters` + `architecture.input_modalities`) and Vercel Gateway public model metadata (`/v1/models` ids/type/tags/context), (3) derive `supportsStructuredOutputs` and chat/vision/reasoning/tool tags, (4) **auto-select lane** via a single `selectReplyFormat(provider, modelInfo)` (OpenRouter: structured iff supported, else Lane B text; Vercel gateway: structured for language models by documented Gateway policy until per-model structured metadata exists), (5) keep a debug override but not as the normal path. THEN TTS. Fish official JS SDK = `fish-audio` (`github.com/fishaudio/fish-audio-typescript`), `FishAudioClient` + `textToSpeech.convertRealtime()` + `RealtimeEvents.AUDIO_CHUNK` — to be added at the TTS slice with `condition_on_previous_chunks=true`.

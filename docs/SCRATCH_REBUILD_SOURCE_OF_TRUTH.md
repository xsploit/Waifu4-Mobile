...
| AI | LLM provider/model/settings, capability metadata, health, generation knobs, tools/reasoning controls. | PARTIAL | Uses rebuilt backend contracts and model metadata. Tool controls and Tavily browser-key storage are present, but main assistant `/ai/chat` does not yet execute AI SDK tools; current live tool execution is GRILLO/Ladybug memory-worker tooling only. This is LLM-adjacent, so restoring assistant web/tool calls needs a deliberate implementation slice. |

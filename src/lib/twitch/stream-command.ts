...
export type DirectStreamCommand =
  | { kind: 'help' }
  | { kind: 'status' }
  | { kind: 'audio' }
  | { kind: 'ai-state' }
...
export function isDirectStreamCommandAuthorized(
  ...
  return (
    ...
    (options.allowTrustedControllers && message.isTrustedController === true)
  );
}
...
export function parseDirectStreamCommand(
  ...
  if (!isDirectStreamCommandAuthorized(message, options)) {
  ...
  if (verb === 'audio') {
    return { matched: true, authorized: true, commandText, command: { kind: 'audio' } };
  }
  ...
  if (['llm', 'model', 'ai'].includes(verb) && rest) {
    return { matched: true, authorized: true, commandText, command: { kind: 'set-ai-model', model: rest } };
  }
  ...
  if (['persona', 'personality', 'profile'].includes(verb) && rest) {
    return { matched: true, authorized: true, commandText, command: { kind: 'set-persona', selector: rest } };
  }
  if (['character', 'char'].includes(verb) && rest) {
    return { matched: true, authorized: true, commandText, command: { kind: 'set-character', selector: rest } };
  }
  if (verb === 'vrms') {
    return { matched: true, authorized: true, commandText, command: { kind: 'list-vrms' } };
  }
  if (['vrm', 'avatar'].includes(verb) && rest) {
    return { matched: true, authorized: true, commandText, command: { kind: 'set-vrm', model: rest } };
  }
  ...
  if (['anim', 'animation', 'dance'].includes(verb)) {
    ...
    if (subcommand === 'speed') {
      ...
      if (Number.isFinite(speed)) {
        return { matched: true, authorized: true, commandText, command: { kind: 'set-animation-speed', speed } };
      }
    }
    ...
    if (rest) {
      return { matched: true, authorized: true, commandText, command: { kind: 'play-animation', selector: rest } };
    }
  }
  if (verb === 'tts') {
    const enabled = parseBoolean(tokens[0]);
    if (enabled !== null) {
      return { matched: true, authorized: true, commandText, command: { kind: 'set-tts', enabled } };
    }
  }
  if (['autospeak', 'autosay'].includes(verb)) {
    const enabled = parseBoolean(tokens[0]);
    if (enabled !== null) {
      return { matched: true, authorized: true, commandText, command: { kind: 'set-auto-speak', enabled } };
    }
  }
  if (verb === 'say' && rest) {
  ...
  if (['chat', 'reply', 'replies'].includes(verb)) {
    const enabled = parseBoolean(tokens[0]);
    if (enabled !== null) {
      return { matched: true, authorized: true, commandText, command: { kind: 'set-chat-overlay', enabled } };
    }
  }

...
- The opt-in `/api/memory/grillo/context/diagnose` endpoint returns a
  deterministic memory sufficiency receipt alongside a provenance-enabled
  packet. It detects correction, temporal, commitment,
...
3. The memory-sufficiency receipt is available only through the opt-in
   diagnostics endpoint. It does not add work to live answers or control reply
   behavior.
